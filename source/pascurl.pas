...
type
  ...
  curl_ssl_ctx_callback = function (curl : CURL; (* easy handle *)
    ssl_ctx : Pointer; (* actually an OpenSSL SSL_CTX *) userptr : Pointer)
    : CURLcode of object;
...
const
  ...
  CURLSSH_AUTH_GSSAPI = 1 shl 5;     (* gssapi (kerberos, ...) *)
...
type
  ...
  curl_khkey = record
    key : PChar; (* points to a zero-terminated string encoded with base64
  ...
  curl_sshkeycallback = function (easy : CURL; (* easy handle *)
    knownkey : pcurl_khkey; (* known *) foundkey : pcurl_khkey; (* found *)
    keys : curl_khmatch; (* libcurl's view on the keys *)
    clientp : Pointer (* custom pointer passed from app *)) : Integer of
    object;
...
const
  CURL_GLOBAL_SSL        = 1 shl 0; (* no purpose since since 7.57.0 *)
...
const
  ...
  CURL_VERSION_PSL       = 1 shl 20; (* Mozilla's Public Suffix List, used
                                             for cookie domain verification *)
  CURL_VERSION_HTTPS_PROXY = 1 shl 21; (* HTTPS-proxy support built-in *)
  CURL_VERSION_MULTI_SSL = 1 shl 22; (* Multiple SSL backends available *)
  CURL_VERSION_BROTLI    = 1 shl 23; (* Brotli features are present. *)
